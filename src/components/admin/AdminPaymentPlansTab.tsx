...
const AdminPaymentPlansTab = () => {
  ...
  const createPlan = async () => {
    if (!newBookingId || newTotal <= 0 || newInstalments < 2) {
    ...
    const { error } = await supabase.from("payment_plans").insert({
      ...
      total_instalments: newInstalments,
      paid_instalments: newFirstPayment > 0 ? 1 : 0,
    ...
    setShowCreate(false);
    setNewBookingId(""); setNewTotal(0); setNewInstalments(3); setNewFirstPayment(0);
    fetchPlans();
  };

  const recordPayment = async (plan: PaymentPlan) => {
    const newPaid = plan.paid_instalments + 1;
    const isComplete = newPaid >= plan.total_instalments;
    const nextDate = isComplete ? null : (() => {
      const d = new Date();
    ...
    toast.success(isComplete ? "Plan completed!" : `Payment ${newPaid}/${plan.total_instalments} recorded`);
    fetchPlans();
  };
        ...
        <div className="border border-gold bg-gold/5 p-6 mb-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select value={newBookingId} onChange={e => {
                setNewBookingId(e.target.value);
                const b = bookings.find(b => b.id === e.target.value);
                if (b) setNewTotal(Number(b.total_price));
          ...
          {newTotal > 0 && newInstalments >= 2 && (
            <div className="bg-secondary p-3 font-body text-xs">
              {newFirstPayment > 0 ? (
                <p>£{newFirstPayment} today, then {newInstalments - 1}x £{((newTotal - newFirstPayment) / (newInstalments - 1)).toFixed(2)}/month</p>
              ) : (
